...
public class DataInitializer {
    ...
    CommandLineRunner initDatabase(EventRepository eventRepo,
        ...
        return args -> {
            ...
            // --- SCHRITT 1: STANDARD-BENUTZER INITIALISIEREN (Nur wenn DB leer ist) ---
            if (userRepo.count() == 0) {
            ...
            // --- SCHRITT 1b: EASTER EGG USER (Immer prüfen, ob er fehlt!) ---
            // Dieser Block läuft unabhängig davon, ob die DB leer ist oder nicht.
            if (userRepo.findByUsername("Doom").isEmpty()) {
                System.out.println("--- Erstelle Easter-Egg User: Doom ---");
                // Passwort 'doom' wird verschlüsselt, Rolle ist 'DOOMSLAYER'
                User doomGuy = new User("Doom", encoder.encode("doom"), "DOOMSLAYER", "Doom Guy");
                userRepo.save(doomGuy);
            }
